...
st.markdown("""
    ...
    .chat-bubble:hover {
        ...
        box-shadow: 
            0px 8px 20px rgba(0,0,0,0.25),   /* Outer deep shadow */
            inset 0px 2px 6px rgba(255,255,255,0.3); /* Soft inner light */
        cursor: pointer;
    }
    </style>
...
def render_message_bubble(sender, message, timestamp, current_user, is_read=False):
    ...
    if sender.lower() == current_user.lower():
        bubble_color = "#FFEFEA"  # Ultra Light Peach
        text_color = "#4B2E2E"    # Soft Brownish Plum
        border_radius = "20px 20px 5px 20px"
    else:
        bubble_color = "#F9F0FF"  # Very Light Lavender
        text_color = "#3A2D42"    # Deep Mauve-Grey
        border_radius = "20px 20px 20px 5px"

    # 💕 Add romantic emoji based on sender
    ...
    <div style="display:flex; justify-content:{align}; margin:6px 0;">
        ...
        ">
            <b style="color:{text_color};">{display_name}</b><br>{message}
