...
def render_message_bubble(sender, message, timestamp, current_user, is_read=False):
    ...
    # Colors + Style (Different for you vs partner)
    if sender.lower() == current_user.lower():
        bubble_color = "linear-gradient(135deg, #FFD6D6, #FFEFEA)"  # Soft pink-peach gradient
        text_color = "#3A1F1F"  # Deep romantic brown
        font_weight = "600"  # Bold for your messages
        border_radius = "24px 24px 8px 24px"
    else:
        bubble_color = "linear-gradient(135deg, #EBDFFF, #F9F0FF)"  # Lavender gradient
    ...
    <div style="display:flex; justify-content:{align}; margin:6px 0;">
        <div class="chat-bubble" style="
            ...
            text-align:center;
        ">
            <b style="font-family:'Dancing Script', cursive; font-size:20px; color:{text_color};">{display_name}</b><br>
            {message}
