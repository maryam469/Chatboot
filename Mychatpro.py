...
def render_message_bubble(sender, message, timestamp, current_user, is_read=False):
    ...
    if sender.lower() == current_user.lower():
        bubble_color = "#FFEFEA"  # Light Peach
        text_color = "#4B2E2E"    # Soft Brown
        border_radius = "24px 24px 8px 24px"
    else:
        bubble_color = "#F9F0FF"  # Light Lavender
        text_color = "#3A2D42"    # Deep Mauve
    ...
    <div style="display:flex; justify-content:{align}; margin:6px 0;">
        <div class="chat-bubble" style="
            ...
            text-align:center;
        ">
            <b style="font-family:'Dancing Script', cursive; font-size:18px; color:{text_color};">{display_name}</b><br>
            {message}
            <div style="font-size:11px; color:gray; text-align:center;">🕒 {timestamp} {ticks}</div>
        </div>
    </div>
    """
